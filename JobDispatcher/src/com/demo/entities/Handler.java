...
public class Handler implements IEntity {
	...
	public String toString() {
		if (getJob() == null)
			return (String.format("%1$s(%2$s,%3$s,%4$s) No Job allocated", getName(), getNameLength(), getVowels(),
					getConsonants()));
		else
			return (String.format("%1$s(%2$s,%3$s,%4$s) Score:%5$s, Street:%6$s(%7$s)", getName(), getNameLength(),
					getVowels(), getConsonants(), getScore(), getJob().getStreetName(),
					getJob().getStreetNameLength()));
	}
}
